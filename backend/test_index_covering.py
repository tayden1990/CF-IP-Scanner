...
async def test_covering_index():
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            print("Adding Cover Index: idx_provider_time_status(provider, timestamp, status)...")
            try:
                await cur.execute("CREATE INDEX idx_provider_time_status ON scan_results(provider, timestamp, status)")
...
if __name__ == "__main__":
    asyncio.run(test_covering_index())
